...
//                                 ) : (
//                                     <Button  className="sm:w-auto w-full" variant="primary"  onClick={undefined}  >
//                                         <p>Submit</p>
//                                     </Button>
//                                 )}
//                             </div>
...
// Assuming UploadBox and Button components are defined elsewhere and imported
// For demonstration, I'll include a basic version of them.
import { Button } from "@/components/ui/button"
const UploadBox = ({ label, handleChange, name }) => (
    <div className="mb-4">
        ...
        <input
            ...
            name={name}
            onChange={handleChange}
            className="w-full bg-[#1E1E1E] border border-gray-700 text-white p-3 rounded-md text-lg file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-yellow-500 file:text-black hover:file:bg-yellow-600"
        />
    </div>
);
...
const Checkbox = ({ id, onChange ,checked}) => (
    <input
        ...
        checked={checked}
        onChange={onChange}
        className="form-checkbox h-5 w-5 text-yellow-500 rounded border-gray-300"
    />
);
...
const RegisterBusinessUi = () => {
    ...
    // const [ninBvnVerified, setNinBvnVerified] = useState(false);
    ...
    const [formData, setFormData] = useState({
        ...
        businessNature: '',
        contactName: '',
        phoneNumber: '', // Renamed from 'phone' to 'phoneNumber' to match API
        contactEmail: '',
        bvn: '',
        nin: '',
        certificateOfIncorporation: null,
        cacForm2_1: null, // This maps to 'certifiedList' in the UI
        proofOfID: null,
        proofOfAddress: null,
        otherDocuments: null,
        declaration: false,
  ...
  const handleSubmit = async () => {
    ...
    try {
        ...
        const verifyResponse = await fetch('https://plugins.cyclebreeze.com/api/seller/kyb/verified', {
            method: 'POST',
            headers: {
        ...
        if (!verifyResponse.ok) {
            ...
            setSubmissionMessage(`Verification failed: ${error.message || ''}`);
            return; // Stop submission
        }

        // If verification is successful, proceed with submission
        const url = `${ApiBaseUrl}/seller/kyb/business`;
        const form = new FormData();

        form.append('business_name', formData.businessName);
        form.append('registration_number', formData.registrationNumber);
        form.append('registration_date', formData.registrationDate);
        form.append('business_address', formData.businessAddress);
        form.append('business_nature', formData.businessNature);
        form.append('firstname', formData.contactName.split(' ')[0] || '');
        form.append('lastname', formData.contactName.split(' ')[1] || '');
        form.append('phone_number', formData.phoneNumber);
        form.append('nin', formData.nin);
        form.append('bvn', formData.bvn);

        if (formData.certificateOfIncorporation) {
            form.append('certificate_of_incorporation', formData.certificateOfIncorporation);
        }
        if (formData.cacForm2_1) {
            form.append('cac_form_2_1', formData.cacForm2_1);
        }
        if (formData.proofOfID) {
            form.append('proof_of_id', formData.proofOfID);
        }
        if (formData.proofOfAddress) {
            form.append('proof_of_address', formData.proofOfAddress);
        }
        if (formData.otherDocuments) {
            form.append('other_documents', formData.otherDocuments);
        }
        ...
        if (response.ok) {
            setSubmissionMessage('Business registration successful!');
            // Reset form or redirect if needed
        } else {
            setSubmissionMessage(`Submission failed: ${data.message || 'Unknown error'}`);
        }
    } catch (error) {
        console.error(error);
                            ...
                            <div>
                                ...
                                <div className="mt-1 flex items-center gap-2">
                                    <PhoneInput className='w-full bg-gray-900 border border-gray-700 rounded-md px-4 py-2 text-lg'
                                        ...
                                        onChange={handlePhoneChange}
                                        inputStyle={{ width: '100%', backgroundColor: '#1A202C', color: 'white', border: 'none' }}
                                        buttonStyle={{ backgroundColor: '#1A202C', border: 'none' }}
                                        dropdownStyle={{ backgroundColor: '#1A202C', color: 'white' }}
                                    />
                                </div>
                            </div>
                            <div>
                ...
                return (
                    <main className="md:w-3/4">
                        <section className="mb-10">
                            ...
                            <p className="text-sm text-gray-400 mb-6">Please upload relevant documents below.</p>
                            <UploadBox label="Certificate of incorporation/registration *" handleChange={handleChange} name="certificateOfIncorporation" />
                            <UploadBox label="Upload Certified List of Shareholders/CAC 2A/Application for Registration of a company/Status Report issued by CAC *" handleChange={handleChange} name="cacForm2_1" />
                        </section>
                        <section className="mb-10">
                            <h3 className="text-lg font-semibold mb-1">Proof of Identification</h3>
                            <p className="text-sm text-gray-400 mb-6">
                        ...
                        <section className="mb-6">
                            ...
                            <UploadBox label="Proof of Address *" handleChange={handleChange} name="proofOfAddress" />
                            <UploadBox label="Any other relevant documents (licenses, certificates, etc.)" handleChange={handleChange} name="otherDocuments" />
                        </section>
                        <div className="mb-6 flex items-start space-x-3">
                            <Checkbox id="declaration" onChange={handleChange} checked={formData.declaration} />
                        ...
                        {submissionMessage && (
                            <p className={`mb-4 text-center ${submissionMessage.includes('successful') ? 'text-green-500' : 'text-red-500'}`}>
                                {submissionMessage}
                            </p>
                        )}
                        <div className="flex flex-col sm:flex-row gap-4 justify-end">
                            <Button variant="outline" className="sm:w-1/2 md:w-auto" onClick={() => setActiveStep(2)}>
                                Go back
                            </Button>
                            <Button className="sm:w-1/2 md:w-auto px-8" onClick={handleSubmit} disabled={loading || !formData.declaration}>
        ...
        <div className="min-h-screen bg-black text-white py-10 px-6 md:px-20">
            ...
            <div className="flex flex-col md:flex-row gap-10">
                <aside className="md:w-1/4 mb-6">
                    <h2 className="text-xl font-semibold mb-6">KYB Verification</h2>
                    <ol className="space-y-4 text-sm text-gray-400">
                        <li className={`flex items-center space-x-2 ${activeStep >= 1 ? 'text-yellow-500' : 'text-gray-500'}`} onClick={() => setActiveStep(1)} >
                            <div className={`w-5 h-5 rounded-full ${activeStep >= 1 ? 'bg-yellow-500 text-black' : 'border border-yellow-500 text-yellow-500'} flex items-center justify-center text-xs font-bold`}>1</div>
                            <span>Business details</span>
                        </li>
                        <li className={`flex items-center space-x-2 ${activeStep >= 2 ? 'text-yellow-500' : 'text-gray-500'}`} onClick={() => setActiveStep(2)}>
                            <div className={`w-5 h-5 rounded-full ${activeStep >= 2 ? 'bg-yellow-500 text-black' : 'border border-yellow-500 text-yellow-500'} flex items-center justify-center text-xs font-bold`}>2</div>
                            <span>Contact details</span>
                        </li>
                        <li className={`flex items-center space-x-2 ${activeStep >= 3 ? 'text-yellow-500 font-medium' : 'text-gray-500'}`} onClick={() => setActiveStep(3)} >
                            <div className={`w-5 h-5 rounded-full ${activeStep >= 3 ? 'bg-yellow-500 text-black' : 'border border-yellow-500 text-yellow-500'} flex items-center justify-center text-xs font-bold`}>3</div>
                            <span>Company/Business documentation</span>
                        </li>
                    </ol>
                </aside>
